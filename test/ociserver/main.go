...
import (
	...
	"github.com/kptdev/porch/test/ociserver/pkg/oci"
	"k8s.io/klog/v2"
)
...
func run(dirs []string) error {
	...
	case 0:
		...
		if err != nil {
			return fmt.Errorf("failed to create temporary directory for OCI repository: %w", err)
		}
	...
	default:
		return fmt.Errorf("can serve only one OCI repository, not %d", len(dirs))
	}

	registries := oci.NewDynamicRegistries(baseDir)

	server, err := oci.NewServer(registries)
	if err != nil {
		return fmt.Errorf("filed to initialize OCI server: %w", err)
	}
