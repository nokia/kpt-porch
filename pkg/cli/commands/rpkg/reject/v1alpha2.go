	...
	for _, name := range args {
		...
		err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
			...
			case porchv1alpha2.PackageRevisionLifecycleProposed:
				...
				err := cliutils.UpdatePackageRevisionApprovalV1Alpha2(r.ctx, r.client, &pr, porchv1alpha2.PackageRevisionLifecycleDraft)
				if err == nil {
					lastErr = nil
				} else {
					lastErr = err
		...
		if err != nil {
			...
			fmt.Fprintf(cmd.OutOrStdout(), "%s no longer proposed for %s\n", name, proposedFor)
		}
	}
	if len(messages) > 0 {
